...
export class ArtifactDeckEncoder {
    ...
    private encodeBytes(deckContents: ArtifactDeck) {
        ...
        for (let unCurrHero = 0; unCurrHero < countHeroes; unCurrHero++) {
            ...
            const card: any = allCards[unCurrHero];
            if (card.turn === 0) {
                throw new Error('Error encoding deck');
            }
        ...
        for (let nCurrCard = countHeroes; nCurrCard < allCards.length; nCurrCard++) {
            ...
            const card: any = allCards[nCurrCard];
            if (card.count === 0) {
                throw new Error('Error encoding deck');
            }
            if (card.id <= 0) {
    ...
    private encodeBytesToString(bytes: number[]) {
    ...
    private addByte(bytes: number[], byte: number) {
        if (byte > 255) {
            throw new Error('Error encoding deck');
        }
        bytes.push(byte);
    ...
    private addRemainingNumberToBuffer(unValue: number, unAlreadyWrittenBits: number, bytes: number[]) {
    ...
    private addCardToBuffer(unCount: number, unValue: number, bytes: number[]) {
        ...
        const unFirstByteCount = bExtendedCount ? knFirstByteMaxCount : /*( uint8 )*/(unCount - 1);
    ...
    private packAndEncode(binArr: number[]) {
            const buff = Buffer.from(binArr);
            const encoded = buff.toString('base64');
            return encoded;
    }

    private computeChecksum(bytes: number[], unNumBytes: number) {
        let unChecksum = 0;
